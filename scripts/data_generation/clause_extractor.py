...
def extract_l1_facts_from_standards(standard_texts, target_count=80):
    ...
    prompt = f"""从以下电力系统标准中提取所有可用于合规核验的规范性条款。包括两类：

1. **数值型参数条款**：含具体数值要求的条款（比例、电压、电流、容量、时间、距离、温度、电阻、频率等）
2. **纯文字规定性条款**：不含数值但属于电气专业的强制性/规定性要求，例如：
   - 设备配置要求（如"应装设两台站用变压器"）
   - 接线方式规定（如"宜采用双母线分段接线"）
...
- 给排水、消防（防火隔墙、灭火装置、消防通道）、暖通空调
- 环保：水土保持、噪声、绿化、碳排放
- 站址与机场/自然保护区/人文遗址的距离

提取格式（纯JSON数组）：
{{"standard":"标准编号","section":"条款号","parameter":"参数名称或规定主题","value":"数值要求或规定内容","clause_text":"完整条款原文","topic":"主题分类","clause_type":"numerical/prescriptive"}}

要求：
- 数值型条款：每条参数一条记录（比例、电压、电流、容量、时间、温度等）
- 文字规定性条款：每条独立的规定性要求一条记录
- 所有条款必须来自标准原文，不编造
- 电气相关条款尽可能全部提取，确保数量充足
